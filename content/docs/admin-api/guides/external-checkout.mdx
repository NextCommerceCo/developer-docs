...
description: Create carts and orders from an external checkout through the Admin API, including upsells, line items, and payment details
...
<Callout type="idea">
Check `supports_post_purchase_upsells` on the initial order create response before attempting an upsell. This boolean tells you whether the order accepts upsells.
</Callout>
