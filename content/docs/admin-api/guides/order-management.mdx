...
description: Edit order line items, refund, update shipping addresses, manage fulfillment orders, add tracking, and cancel orders through the Admin API
...
import SplitFulfillmentOrders from '../../../_snippets/_splitting-fulfillment-orders.mdx';
  ...
  "refund_lines": [
    {
      ...
      "location_id": 12, // location these items are refundable at, see notes
...
<Callout type="info" title="One Refund Line per Location">
...
```json title="Order Create Refund Request" http-method="POST" http-target="https://{store}.29next.store/api/admin/orders/{number}/refund/"
{
  ...
  "refund_lines": [
    ...
    },
    {
      "line_id": 1001,
      "quantity": 2,
      "restock_type": "cancel", // restocks, so location_id is required
      "location_id": 12 // where the items are restocked (from calculate step)
    }
  ],
  "refund_shipping": {
    "amount_excl_tax": "3.99",
...
```
<Callout type="warn" title="location_id Is Conditionally Required">
Restocking needs a destination, so [ordersRefundCreate](/docs/admin-api/reference/orders/ordersRefundCreate) rejects a refund line without `location_id` when `restock_type` is `return` or `cancel`. It stays optional for `no_restock`, and optional on [ordersRefundCalculateCreate](/docs/admin-api/reference/orders/ordersRefundCalculateCreate), where it narrows the response to one location.

Unfulfilled items always restock as `cancel`, so any refund of unshipped items needs it.
</Callout>
