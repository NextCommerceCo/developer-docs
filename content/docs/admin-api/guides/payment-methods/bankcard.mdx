...
<Callout type="info">
Pass either `payment_gateway` or `payment_gateway_group`, not both. If you pass neither, the store's default routing applies. To see which group that is, look for `"is_default": true` in [gatewayGroupsList](/docs/admin-api/reference/payments/gatewayGroupsList).
</Callout>
...
A failed payment returns a `400` with the response code in `payment_response_code`. When the card was tokenized, `payment_method.card_token` is returned so the charge can be reattempted without collecting the card again.

```json title="Failed Payment Response"
{
  "payment_details": "Your card was declined.",
  "payment_method": {
    "card_token": "01M2PQPJCDPEFVDWCK3H353YNB"
  },
