...
Every product is one of three structures, returned as `structure` on the product.

| Structure | What it is | Where it appears |
| --- | --- | --- |
| `standalone` | A product sold in one form with a single variant | Product list and detail |
| `parent` | A product with variant attributes such as Color and Size | Product list and detail |
| `child` | One purchasable variant of a parent | The `variants` array of its parent |

The parent holds the title, description, images, categories and `variant_attributes`. Each variant holds the `sku`, `prices`, `stockrecords`, `track_stock`, `allow_backorders` and its own `image` chosen from the parent's images. A standalone product follows the same split: the product carries the content and its single default variant carries the SKU, price and stock.
...
Create a standalone product with a single POST to the [productsCreate](/docs/admin-api/reference/products/productsCreate) endpoint. Send no `variant_attributes` and exactly one entry in `variants` carrying the SKU, price and stock.

```json title="Create Standalone Product" http-method="POST" http-target="https://{store}.29next.store/api/admin/products/"
{
  "title": "Classic Tee",
...
The response is the full product with `id`, `slug`, `url` and the created variant in `variants`. Without `variant_attributes` the product gets one default variant, and sending more than one entry in `variants` is rejected with "Default product must be only one."
...
Images belong to the parent or standalone product. Upload one from a URL with `src`, or from base64 data with `attachment`, using the [productsImageCreate](/docs/admin-api/reference/products/productsImageCreate) endpoint. Send one or the other, not both.
...
```json title="Organize Product" http-method="PATCH" http-target="https://{store}.29next.store/api/admin/products/{id}/"
{
  ...
  "recommended_products": [2100, 2104], // standalone or parent product ids
...
`recommended_products` accepts standalone and parent product IDs only. Passing a variant `id` is rejected.
</Callout>
