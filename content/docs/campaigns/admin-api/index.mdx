...
An offer discounts the order once its condition is met. `name`, `condition`, and `benefit` are required. The condition decides which packages and what quantity trigger the offer, and the benefit sets the percentage off. An offer applies automatically at checkout unless `offer_type` is `voucher`.
...
A voucher offer applies only when the customer enters its `code`. Set `all_packages` to `true` with an `any` condition to make it valid on anything in the cart.

```json title="Create Voucher Code" http-method="POST" http-target="https://{store}.29next.store/api/admin/campaigns/{id}/offers/"
{
  "name": "Welcome 10% Off",
  "offer_type": "voucher", // offer (automatic) or voucher (code required)
  "code": "WELCOME10", // required when offer_type is voucher
  "condition": {
