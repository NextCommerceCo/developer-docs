...
For most projects, reach for the **[Campaign Cart SDK](/docs/campaigns/cart-system)** before calling this API directly. The SDK wraps every endpoint on this page behind data-attribute driven add-to-cart, live cart state, coupon handling, checkout forms, and upsell flows — so you write HTML, not fetch calls. The [JavaScript API](/docs/campaigns/javascript-api) reference covers the programmatic surface when you need it.
