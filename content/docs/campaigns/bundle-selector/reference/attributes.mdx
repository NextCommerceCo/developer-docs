...
| `data-next-selected="true"` | Pre-selects this card on init. Overridden by `?forceBundleId=` URL parameter. See [Mechanics → Default Selection on Init](../mechanics#default-selection-on-init) |
...
| `excludeProperties` | string | Comma-separated property keys to exclude from the order API for this slot (`"team, number"`), or `"*"` to exclude all properties |

---

## Line Item Properties

Place `data-next-property` on `<input>`, `<textarea>`, or `<select>` elements inside a slot template to attach user input as metadata to that line item.

| Attribute | Description |
|-----------|-------------|
| `data-next-property="<key>"` | Attaches this input's value to the line item as a custom property under the given key |
| `data-next-default-property="<key>"` | Attaches this input's value as a custom property to every line item across all bundle selectors |
| `data-next-exclude-property="<keys>"` | Comma-separated keys to exclude from the order API for this slot, or `"*"` to exclude all |

Inside a slot template:
...
<template id="slot-tpl">
  <div class="slot-row">
    ...
    <input type="text"
           ...
           placeholder="Personalisation text" />
  </div>
</template>
```

For a property that applies to every slot — such as a gift message — place `data-next-default-property` outside the slot template anywhere on the page. A `data-next-property` with the same key within a slot takes precedence.

```html
<input type="text"
       data-next-default-property="gift_message"
...
Row tokens: `{discount.name}`, `{discount.amount}`, `{discount.percentage}`, `{discount.description}`.

`{discount.percentage}` renders the discount's percentage value formatted with a `%` suffix (e.g. `"15%"` or `"15.50%"`). Empty when the discount has no percentage set.
