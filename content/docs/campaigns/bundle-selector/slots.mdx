...
Add `data-next-property-key` to any `<input>`, `<textarea>`, or `<select>` inside a slot template to capture per-slot metadata and attach it to the corresponding line item in the cart.
...
For properties that apply to every slot across all bundles (e.g. a single gift message), use `data-next-default-property-key` anywhere on the page instead:

```html
<!-- Applies to every line item from every bundle selector on the page -->
<textarea data-next-default-property-key="gift_message"
...
Per-slot `data-next-property-key` values override page-level defaults when both share the same key.

Merge priority (highest wins): slot-level `data-next-property-key` → page-level `data-next-default-property-key`.
