...
Place **inside** a `data-summary-lines` row template to render one row per line item property.
  ...
  <template>
    <li>
      ...
      <div data-next-item-properties>
        <template>
          <span class="property">{property.key}: {property.value}</span>
        </template>
      </div>
    </li>
  </template>
...
Items that share the same `package_id` but carry different properties are rendered as separate rows, each with their own `data-next-item-properties` list.
...
| `discount.percentage` | string | Discount percentage formatted with `%` (e.g. `"15%"` or `"15.50%"`). Empty when the discount has no percentage value. |
