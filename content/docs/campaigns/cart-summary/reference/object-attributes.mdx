...
One entry in the `data-next-item-properties` list rendered inside a `data-summary-lines` row. Only present when the line item carries custom properties (e.g. personalisation text collected via `data-next-property-key`).

| Token | Type | Description |
|-------|------|-------------|
| `{property.key}` | string | Property name (e.g. `"engraving"`) |
| `{property.value}` | string | Property value as entered by the shopper |
