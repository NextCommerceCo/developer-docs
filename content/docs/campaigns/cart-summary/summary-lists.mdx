...
When a line item carries custom properties (personalisation fields collected via `data-next-property`), add a `data-next-item-properties` container inside the row template. The SDK renders one child row per property without any hardcoded keys.
...
<ul data-summary-lines>
  <template>
    <li>
      ...
      <!-- Renders one row per property on this line -->
...
| Token | Description |
|-------|-------------|
| `{property.key}` | Property name (e.g. `"engraving"`) |
| `{property.value}` | Customer-entered value (e.g. `"Happy Birthday"`) |

**State classes on the container:**

| Class | When |
|-------|------|
| `next-summary-empty` | This line has no properties |
| `next-summary-has-items` | One or more properties are present |

Use `.next-summary-empty { display: none }` to hide the container when the line has no properties.

When two slots share the same `package_id` but have different properties (e.g. two shirts with different engraving text), the cart renders them as separate line items — each row has its own `data-next-item-properties` output.
