...
| `cart.hasCoupon()` | `true` when any coupon is applied to the cart |
| `cart.hasCoupon("CODE")` | `true` when the specified coupon code is applied. Codes are compared case-insensitively; surrounding quotes are stripped. |
...
<div data-next-show="cart.hasCoupon()">
  Coupon applied — see your savings in the summary.
</div>

<!-- Specific coupon -->
<div data-next-show='cart.hasCoupon("FREESHIP")'>
  Free shipping unlocked.
</div>
```

<Callout type="info">
`cart.hasCoupon` (no parens) returns `true` for any coupon — same as `cart.hasCoupon()`. Use the function form when checking a specific code.
</Callout>
