...
1. Pick a starter template (Olympus, Demeter, multi-variant, two-step, and more)
...
Pick one of these at the `campaign-init` template prompt. Each checkout template ships with `presell.html` + `landing.html`, all upsell variants, and a receipt — copy the whole folder to get the full funnel.
...
Browse the full catalog and source in the [campaign-cart-starter-templates repo](https://github.com/NextCommerceCo/campaign-cart-starter-templates).
