...
| `data-next-quantity` | Quantity to add when toggling on (default `1`); ignored when `data-next-package-sync` or `data-next-product-sync` is set |
...
| `data-next-product-sync` | Product ID — quantity mirrors the sum of **all variants** of that product in the cart. Use for configurable (multi-variant) products. See [Quantity Sync → Syncing Across All Variants](../quantity-sync#syncing-across-all-variants--data-next-product-sync) |
| `data-next-property` | On an `<input>`, `<textarea>`, or `<select>` inside the card — attaches the field value to the line item as a named property (e.g. `data-next-property="engraving"`) |
| `data-next-default-property` | On any input outside a card — applies the field value as a property to every toggle line item. Per-card `data-next-property` values with the same key take precedence |
| `data-next-exclude-property` | Comma-separated property keys to exclude from the order API for this card (e.g. `"team, number"`), or `"*"` to exclude all properties. Can be set on the card element, its state container, or via `data-next-packages` JSON as `excludeProperties` |
...
Row placeholders: `{discount.name}`, `{discount.amount}`, `{discount.percentage}`, `{discount.description}`.
