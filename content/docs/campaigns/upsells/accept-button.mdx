...
Any `data-next-property` or `data-next-default-property` inputs present on the upsell page are included in the `lines[].properties` payload sent to the order API — the same way they work on the main checkout page. No extra configuration is required.

```html
<!-- Collected and sent with the upsell -->
<input data-next-property="engraving" placeholder="Personalisation text" />
...
Use `data-next-exclude-property` on a PackageToggle card if you need to suppress specific keys for that upsell item.
