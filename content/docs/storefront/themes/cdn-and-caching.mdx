...
Always use the store network domain `{store}.29next.store` when developing, previewing, debugging, and verifying themes. Do not use a mapped public storefront domain to decide whether a theme change landed.
</Callout>
### Asset CDN
...
<Callout type="idea" title="Verify the Network Domain First">
Confirm the latest theme on `{store}.29next.store` first. After it is correct there, check the mapped public storefront domain as a final customer-path smoke test.
</Callout>
### Template Caching
