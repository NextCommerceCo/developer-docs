...
After uploading and selecting the template, open the page on `https://{store}.29next.store` and verify the served HTML, expected assets, and behavior. Do not use a mapped public storefront domain to decide whether the template change landed.
