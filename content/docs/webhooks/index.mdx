...
There is no separate chargeback event. `dispute.created` and `dispute.updated` cover chargebacks and pre-chargeback alerts alike; the dispute's `type` distinguishes them. See the [Disputes guide](https://docs.nextcommerce.com/docs/features/payments/disputes-guide).
</Callout>
